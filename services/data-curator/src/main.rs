...
use crate::{
    infrastructure::{
        database::{PgVectorEmbeddingRepository, PostgresCurationJobRepository},
        embeddings::{ClipEmbeddingService, MockEmbeddingService},
        storage::{LocalStorageService, S3StorageService},
    },
    presentation::grpc_services::{
        DataCuratorGrpcService,
        data_curator_proto::data_curator_service_server::DataCuratorServiceServer,
    },
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let storage_service: Arc<dyn application::StorageService> =
    ...
    let embedding_service: Arc<dyn application::EmbeddingService> =

...
use crate::{
    application::{
        StartCurationUseCase, GetCurationStatusUseCase, SubmitFeedbackUseCase,
        StorageService, EmbeddingService,
    },
    domain::{
        models::{ImageFeedback as DomainImageFeedback, ImageForFeedback as DomainImageForFeedback},
...
use data_curator_proto::{
    ...
    StartCurationRequest, StartCurationResponse,
    GetCurationStatusRequest, CurationStatus,
    SubmitFeedbackRequest, SubmitFeedbackResponse,
    JobStatus, ImageForFeedback, Feedback,
};
    ...
    EM: EmbeddingService + Send + Sync + 'static,
{
    pub fn new(
        ...
        let get_status_use_case = Arc::new(GetCurationStatusUseCase::new(
            Arc::clone(&job_repository),
        ));
    ...
    EM: EmbeddingService + Send + Sync + 'static,
{
    async fn start_curation(
        ...
        let job_id = self.start_curation_use_case
    ...
    async fn get_curation_status(
        ...
        let job = self.get_status_use_case
            .execute(job_id)
            .await
            .map_err(|e| {
        ...
        match job {
            Some(job) => {
                let status = match job.status {
                    ...
                    crate::domain::models::CurationStatus::AwaitingFeedback => JobStatus::AwaitingFeedback,
                ...
                let images_for_feedback: Vec<ImageForFeedback> = job.images_for_feedback
                    .into_iter()
    ...
    async fn submit_feedback(
        ...
        let feedback: Vec<DomainImageFeedback> = req.feedback
            .into_iter()
        ...
        let acknowledged = self.submit_feedback_use_case

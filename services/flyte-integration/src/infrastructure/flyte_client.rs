...
impl FlyteClientTrait for HttpFlyteClient {
    ...
    async fn get_execution_status(&self, execution_id: &str) -> Result<String, anyhow::Error> {
        let url = format!(
            "{}/api/v1/executions/{}/{}",
            self.flyte_admin_url, self.project, execution_id
        );
...
impl FlyteConfig {
    ...
    pub fn from_env() -> Self {
        Self {
            ...
            project: std::env::var("FLYTE_PROJECT").unwrap_or_else(|_| "flytesnacks".to_string()),
            domain: std::env::var("FLYTE_DOMAIN").unwrap_or_else(|_| "development".to_string()),
